...
vector<osobnik> GA::fitness(vector<osobnik>o,double sum) {
	vector<osobnik>temp_population;
	int fitness_size{ 0 };
	double fit_mid{ 0 };
	for (int i{ 0 }; i < size_pop; i++) {
		o[i].fitness = 0;
		double fit{ (o[i].value * 100) / sum };  //rocetowy udzial w sumie wartosci 
		int F{ int(fit) };
		double X = fit - F;
		o[i].fitness = F;
